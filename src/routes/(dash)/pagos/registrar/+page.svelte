<script lang="ts">
  ...
  function submitForm() {
    ...
    // y procesar los datos en función de los valores seleccionados.
    ...
    <form class="bg-white shadow-md rounded-lg px-8 pt-6 pb-8 mb-4">
      ...
      <div class="mb-4">
        <label
          ...
          for="cedula_estudiante">Cédula del Estudiante</label
        >
        <select
          bind:value="{selectedCedula}"
          class="select px-3 py-2 rounded-lg"
          id="cedula_estudiante"
        >
          <option value="" disabled>Selecciona una opción</option>
          {#each cedulas as cedula (cedula.id)}
            <option value="{cedula.id}">{cedula.value}</option>
          {/each}
        </select>
      </div>
      <div class="mb-4"><label for="fecha" class="text-gray-700 font-bold mb-2 label">Fecha del Pago</label>
      <DatePicker
          format="%d-%m-%Y"
          right="{true}"
      ...
      <div class="mb-4">
        <label class="label text-gray-700 font-bold mb-2">Método de Pago</label>
        <RadioGroup
      ...
      {#if selectedMetodo === "Efectivo"}
        <div class="mb-4 flex justify-start items-center gap-x-5">
          ...
          <button class="btn variant-filled-primary w-fit h-fit">Añadir billete</button>
        </div>
      {/if}
